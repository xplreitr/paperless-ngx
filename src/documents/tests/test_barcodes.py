...
class TestBarcode(
    ...
    def test_scan_file_for_separating_barcodes_password(self):
        ...
        with self.assertLogs("paperless.BarcodePlugin", level="WARNING") as cm:
            with self.get_reader(test_file) as reader:
                reader.detect()
                warning = cm.output[0]
                expected_str = "WARNING:paperless.BarcodePlugin:File is likely password protected, not checking for barcodes"
